// Netlify Function:创建新 token
// POST { } (不需要参数)
// 返回: { ok, token: {id, created, devices: []} }
exports.handler = async (event) => {
  ...
  const newToken = { id, created: new Date().toISOString(), devices: [] };
  db.tokens[id] = newToken;

  // 写
  const content = Buffer.from(JSON.stringify(db, null, 2), 'utf-8').toString('base64');
  const putBody = { message: `[fucai3d create] ${id}`, content, branch: GH_BRANCH };
  ...
  return { statusCode: 200, headers, body: JSON.stringify({ ok: true, token: newToken }) };
};
